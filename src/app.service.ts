...
export class AppService {
  ...
  async crawl() {
    ...
    //   }
    // } catch (error) {
    //   console.log('요소가 존재하지 않습니다.', { error });
    // }
  ...
  async crawl2() {
    ...
    await page.waitForNavigation();
    await page.goto(
      'https://www.nhis.or.kr/nhis/healthin/retrieveHealthinCheckUpTargetResultPerson.do',
    );
    ...
    page.on('response', async (response) => {
      ...
      console.log(
        `Response: ${response.url()} Status: ${response.status()} Type: ${response.request().resourceType()}`,
      );
      if (response.request().resourceType() === 'media') {
        ...
        const transcript = (await whisper(`${filePath} --no-gpu`, {
          ...options,
        ...
        await page.click(
          'xpath=//*[@id="frm_captcha_auth"]/div[2]/div[2]/a[1]',
        );
        await page.waitForNavigation();
        await page.goto(
          'https://www.nhis.or.kr/nhis/healthin/retrieveHealthinCheckUpTargetResultAllPerson.do',
        );
        await page.waitForSelector(
          'xpath=//*[@id="cms-content"]/div[3]/div[1]/a[3]',
        );
